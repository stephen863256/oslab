...
size_t heap_size = 0;
double get_utilization() {
    return (double)((1.0*user_malloc_size)/heap_size); 
}
/* 
...
static void* find_fit_best(size_t asize) {
    ...
    */
    ...
    while(bp!= NULL)
    {
        ...
        if(bp_size > asize)
        {
            if(temp == NULL || bp_size < GET_SIZE(HDRP(temp)))
                temp = bp;
        }
        bp = (char*)GET_SUCC(bp);
    }
...
static void place(void *bp, size_t asize)
{
    ...
    */
    ...
    //printf("place really begin!\n");
    size_t bp_size = GET_SIZE(HDRP(bp));
    //size_t prev_alloc = GET_PREV_ALLOC(HDRP(bp));
    //size_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(bp)));
    //printf("%llu\n",bp_size);
    if((bp_size-asize) < MIN_BLK_SIZE)//分配整个空闲块
    {    
        //printf("%d\n",j++);
    ...
    {
        //printf("place about to end!\n");
        //printf("%d\n",j++);
        PUT(HDRP(bp),PACK(asize,1,1));
        //printf("place really about to end!\n");
        char *next_bp = NEXT_BLKP(bp);
        //printf("%p\n",bp);
        PUT(HDRP(next_bp),PACK(bp_size-asize,1,0));
         //printf("%p\n",next_bp);
         //printf("place really really about to end!\n");
        PUT(FTRP(next_bp),PACK(bp_size-asize,1,0));
        add_to_free_list(next_bp);
       
    }
    //printf("place real success!\n");
