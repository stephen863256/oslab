...
cluster_t read_fat_entry(cluster_t clus)
{
    ...
    /** Your Code Here ... **/
    cluster_t fat16_value;
    if(is_cluster_inuse(clus))
    {
        size_t offset = clus*2 + meta.reserved * meta.sector_size;
        sector_t sec_num = offset / meta.sector_size;
        printf("1\n");
        sector_read(sec_num,sector_buffer);
        size_t sec_offset = offset % meta.sector_size;
        memcpy(&fat16_value,sector_buffer + sec_offset,sizeof(fat16_value));
        return fat16_value;
    }
    exit(-1);
}
...
int find_entry_in_sectors(const char* name, size_t len, 
    ...
    for(size_t i = 0; i < sectors_count; i++) {
        ...
        for(size_t off = 0; off < meta.sector_size; off += DIR_ENTRY_SIZE) {
           ...
           char longname[12];
            //to_longname(dir->DIR_Name,longname,len+1);
            char name0[12];
            //strncpy(name0,name,len);
            //name0[len] = '\0';
            printf("%s   %d  namenamename\n",name0,len);
            ...
            if(check_name(name,len,dir)&& is_valid(dir))
            {
                slot->dir = *dir;
                slot->offset = off;
...
/**
 * @brief 文件系统初始化，无需find_empty_slot修改
 * 
 * @param conn 
 * @return void* 
 */
    ...
    while (root || is_cluster_inuse(clus)) {
        ...
        for(size_t i=0; i < nsec; i++) {
            ...
            for(size_t off = 0; off < meta.sector_size; off += DIR_ENTRY_SIZE) {
                ...
                if(is_valid(dir) &&(dir->DIR_Attr == ATTR_DIRECTORY || 
                dir->DIR_Attr == ATTR_REGULAR))
                {
                    char longname[12];
                    to_longname(dir->DIR_Name,longname,11);
                    printf("%s  dir->DIR_Attr = %d\n",longname,dir->DIR_Attr);
...
int fat16_read(const char *path, char *buffer, size_t size, off_t offset,
    ...
    size_t size_0 = size ;
    size_t offset_0 = offset % (meta.cluster_size);
      size_t len;
      size_t pos = 0;
      cluster_t off = offset/(meta.cluster_size);
      while(pos < size) {
        //printf("%lld %lld %d\n",offset_0,off,clus);
        len = min(meta.cluster_size - offset_0,size - pos);
        read_from_cluster_at_offset((clus+off),offset_0,buffer+pos,len);
        pos += len;
        offset_0 = 0;
        off ++ ;
    }
...
int write_fat_entry(cluster_t clus, cluster_t data) {
    ...
    size_t FAT_sec_offset[meta.fats];
    FAT_sec_offset[0] = meta.reserved;
    FAT_sec_offset[1] = (meta.sec_per_fat + meta.reserved);
    for(size_t i = 0; i < meta.fats; i++) {
        ...
        //   2. 读取该扇区并在对应位置写入数据
        //   3. 将该扇区写回
        size_t sec_num = FAT_sec_offset[i] + clus_sec;
        sector_read(sec_num,sector_buffer);
...
int fat16_mkdir(const char *path, mode_t mode) {
    ...
    ret = dir_entry_create(slot, DOT_NAME, ATTR_DIRECTORY, 0, 0);
    if(ret < 0) {
        return ret;
    }
    printf("%d   first_clus dot\n",first_clus);
    slot.offset += DIR_ENTRY_SIZE;
    ret = dir_entry_create(slot, DOTDOT_NAME, ATTR_DIRECTORY, 0, 0);
    if(ret < 0) {
        return ret;
    }
    printf("%d   first_clus dotdot\n",first_clus);
    return 0;
}
...
ssize_t write_to_cluster_at_offset(cluster_t clus, off_t offset, const char* data, size_t size) {
    printf("Write clus %hd at offset %ld, size: %lu\n", clus, offset, size);
    assert(offset + size <= meta.cluster_size);  // offset + size 必须小于簇大小
    char sector_buffer[PHYSICAL_SECTOR_SIZE];
    ...
    size_t sec_off = offset % meta.sector_size;
    while(pos < size) {
        ssize_t ret = sector_read(sec,sector_buffer);
        if (ret < 0 )
        {
            return ret;
        }
...
int file_reserve_clusters(DIR_ENTRY* dir, size_t size) {
    ...
    size_t n = size / (meta.cluster_size);
    n = (size % (meta.cluster_size)) ? (n + 1) : n;
    cluster_t first_clus ;
    if(dir -> DIR_FstClusLO == CLUSTER_END)
    {
        alloc_clusters(n,&first_clus);
        dir -> DIR_FstClusLO = first_clus;
    }
    else 
    {
        ...
        if(n > cnt)
        {
            alloc_clusters(n - cnt,&first_clus);
            write_fat_entry(clus,first_clus);
        }
    }
...
int fat16_write(const char *path, const char *data, size_t size, off_t offset,
    ...
    /*
    1. 通过 path 获取到对应的目录项，即 DIR_ENTRY 结构体，通过调用 find_entry 即可（任务一中
实现）。
    2. 比较新写入的数据和文件原来的数据所需的簇数量，如果新写入的数据所需的簇数量大于原来所需
的簇数量，则需要为文件扩容簇数量。即在FAT表中查找未分配的簇，并链接在文件末尾。如果你
完成了前置任务中的 alloc_clusters ，你可以在此处使用它，为你分配你所需要的簇数量，你只
需要正确地将分配好的簇链连接在你文件的末尾即可。（如果文件原来没有任何簇，你要把簇链连
接在目录项的 DIR.FstClusLo ，否则，连接在原文件簇链的末尾。）
    3. 按需扩容后，只需要实现写文件操作即可，通过 DIR.FstClusLo 获取第一个簇号，之后可以通过
链接依次遍历，找到要写入簇的位置，并在读取相应的扇区，修改扇区的正确位置，并写回扇区。
（这个过程和 read 的实现很类似。）
    4. 更新对应的目录项，上述过程中，可能修改了目录项的 FstClusLo ，最后，你还需要更新
FileSize ，然后将更改后的目录项写回镜像文件。（使用 dir_entry_write ）
    5. 返回成功写入的数据字节数。
*/
    if(path_is_root(path)) {
        return -EISDIR;
    ...
    while(pos < size) {
        //printf("%lld %lld %d\n",offset_0,off,clus);
        len = min(meta.cluster_size - offset_0,size - pos);
        pos += write_to_cluster_at_offset((clus+off),offset_0,data+pos,len);
        offset_0 = 0;
        off ++ ;
    }
    p = pos;
    if(pos + offset > dir->DIR_FileSize)
        dir->DIR_FileSize = (pos + offset);
...
int fat16_truncate(const char *path, off_t size, struct fuse_file_info* fi) {
    ...
    2. 计算并比较原文件拥有的簇数量 n1 ，和文件截断后需要的新簇数量 n2 。通过比较 n1 和 n2 的大
小，来判断文件是否需要新增或释放簇：
        1. n1 == n2 ：此时不需要任何操作
        2. n1 < n2 ：此时需要扩容文件大小，可以参照 fat_write 实现思路。
        3. n1 > n2 ：此时需要截断文件，找到文件的第 n2 个簇，更改FAT表项，将此处改为文件末
尾，并释放后续所有簇。（释放可使用Part1中的 free_clusters 函数。）
    3. 按需写入，比较新文件的大小 new_size （就是参数中的 size ） 和原来的文件大小 old_size :
        1. new_size > old_size ：需要从 old_size 位置开始，将后续文件数据清空为 0x00 。如果
你正确实现了 alloc_clusters ，那么新分配的簇应该已经清空，而已有的最后一个簇的末
尾也在上一次分配时清空过。
        2. new_size <= old_size ：不需要进行任何操作
        3. 实现目录项的更新，通过 find_root 可以获取到目录项对应的偏移量，更新对应的目录项数据写
入即可
    ...
    size_t n2 = size / (meta.cluster_size) ;
    n2 = (size % (meta.cluster_size)) ? (n2 + 1) : n2; 
        
    if(n1 < n2)
    {
        file_reserve_clusters(dir,size);
    }
    else if(n1 > n2)
    {
        size_t n3 = 1;
        cluster_t clus = dir -> DIR_FstClusLO;
        while(is_cluster_inuse(read_fat_entry(clus)) && n3 != n2)
        {
            n3 ++;
            clus = read_fat_entry(clus);
        }
