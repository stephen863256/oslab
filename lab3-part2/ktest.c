...
// 定义要输出的文件
#define OUTPUT_FILE "/home/yxc/oslab/lab3-part2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffffa4b096f0;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffffa4aeb7e0;
...
static void print_mm_active_info(void)
{
    ...
    if (mm)
    {
        ...
        {
            unsigned long virt_addr = (unsigned long) vma->vm_start;
            while(virt_addr < vma->vm_end)//遍历vma中的虚拟页面
            {
                struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);//获取页面结构体 struct page
                ...
                virt_addr += PAGE_SIZE;//间隔PAGE_SIZE
            }
            vma = vma->vm_next;
        }
        flush_buf(1);
...
static void traverse_page_table(struct task_struct *task)
{
    ...
    if (mm)
    {
        ...
        {
            unsigned long virt_addr = (unsigned long) vma->vm_start;
            for(;virt_addr <(unsigned long) vma->vm_end;virt_addr += PAGE_SIZE)
            {
...
static void print_seg_info(void)
{
    ...
    if (ktest_func == 4) {
        ...
        start_data = mm->start_code;
        end_data = mm->end_code;
    }
    ...
    while(vma)
    {
        ...
        {
            struct page *page = mfollow_page(vma, virt_addr, FOLL_GET);
            unsigned long virt_end_addr = virt_addr + PAGE_SIZE;
            if(!IS_ERR_OR_NULL(page))
            {     
                 if((virt_addr >= start_data)&&(virt_end_addr <= end_data)){//整页都是数据段
                ...
                else if((virt_addr <= start_data)&&(virt_end_addr >= end_data)){//数据段在页面起始地址以后且后面都是数据段
                    addr = kmap_atomic(page);
                ...
                else if((virt_addr <= start_data)&&(virt_end_addr >= start_data)){//数据段在页面起始地址以后且后面不是都是数据段
                    addr = kmap_atomic(page);
                    memcpy(buf, addr + start_data - virt_addr, virt_end_addr - start_data);
                    kunmap_atomic(addr);
                    curr_buf_length += (virt_end_addr - start_data);
                    flush_buf(1); 
                }
                else if((virt_addr <= end_data)&&(virt_end_addr >= end_data)){//数据段在页面起始地址以前且后面不都是数据段
                    addr = kmap_atomic(page);
