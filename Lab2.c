...
int exec_builtin(int argc, char**argv, int *fd) {
    ...
    if (strcmp(argv[0], "cd") == 0) {
        ...
        {
            if(chdir(argv[1]) == -1)
               { printf("shell: cd: /path/to/%s: No such file or directory",argv[1]);
               return -1;}
            else return 0;
        }
        else 
...
int main() {
    ...
    while (1) {
        ...
        int clip_sum = split_string(cmdline,";",multi_commands);//分割出的命令数
        for(int i =0;i < clip_sum;i ++){
        ...
        if(cmd_count == 0) {
            ...
            //for(int t = 0;t < cmd_count;t ++) printf("%s\n",commands[t]);
            // 子进程1
            int pid = fork();
